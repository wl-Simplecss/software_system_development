...
public class LendController {
    ...
    public String ReaderbookLendDo(HttpServletRequest request,RedirectAttributes redirectAttributes,int readerId){
        System.out.println("ReaderbookLendDo"+readerId);
        long bookId=Integer.parseInt(request.getParameter("bookId"));
        boolean lendsucc=lendService.bookLend(bookId,readerId);
        if (lendsucc){
            redirectAttributes.addFlashAttribute("succ", "图书借阅成功！");
    ...
    public String ReaderbookReturn(HttpServletRequest request,RedirectAttributes redirectAttributes,int readerId){
        long bookId=Integer.parseInt(request.getParameter("bookId"));
        boolean retSucc=lendService.ReaderbookReturn(bookId,readerId);
        if (retSucc){
            redirectAttributes.addFlashAttribute("succ", "图书归还成功！");
            return "redirect:/reader_main.html";
        }
